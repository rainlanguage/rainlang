...
/// is greater than the item after it, else 0. Comparisons are chained, so two
/// inputs is the binary case, `greater-than(a b)`, and more than two inputs
/// holds when every adjacent pair holds, e.g. `greater-than(a b c)` is
/// `a > b > c`.
library LibOpGreaterThan {
    using LibDecimalFloat for Float;

    /// @notice `greater-than` integrity check. Requires at least 2 inputs and
    /// produces 1 output.
    ...
    /// @notice GT
    /// GT is 1 if every item is greater than the item after it, else 0.
    /// @param operand Low 4 bits of the high byte encode the input count.
