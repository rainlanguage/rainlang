...
/// is greater than or equal to the item after it, else 0. Comparisons are
/// chained, so two inputs is the binary case,
/// `greater-than-or-equal-to(a b)`, and more than two inputs holds when every
/// adjacent pair holds, e.g. `greater-than-or-equal-to(max x min)` is
/// `max >= x >= min`.
library LibOpGreaterThanOrEqualTo {
    using LibDecimalFloat for Float;

    /// @notice `greater-than-or-equal-to` integrity check. Requires at least 2
    ...
    /// @notice GTE
    /// GTE is 1 if every item is greater than or equal to the item after it,
    /// else 0.
    /// @param operand Low 4 bits of the high byte encode the input count.
