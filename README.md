...
Bytecode correctness is validated once at deploy time by `integrityCheck2`.
Each opcode's `integrity()` function reports accurate stack IO (inputs and
outputs) to the checker, which enforces that the bytecode-declared IO
matches, the stack never underflows, and the final depth is correct.
